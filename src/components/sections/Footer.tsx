...
export default function Footer() {
  ...
  return (
    <footer
      id="blessings"
      ref={ref}
      className="relative z-1 mt-0 overflow-hidden px-[clamp(16px,3vw,32px)] pt-[clamp(24px,4vh,40px)] pb-[max(88px,calc(64px+env(safe-area-inset-bottom,0px)))] text-center text-gold-pale scroll-mt-[76px] bg-[radial-gradient(ellipse_at_50%_100%,rgba(185,137,62,0.08),transparent_48%),#1a060c] max-[1100px]:z-3 max-[720px]:px-4 max-[720px]:pt-5 max-[720px]:pb-[max(80px,calc(56px+env(safe-area-inset-bottom,0px)))] max-[720px]:[&_.petal:nth-child(n+5)]:hidden max-[720px]:[&_.sparkle:nth-child(n+5)]:hidden"
    >
      <div
      ...
      <div className="relative z-3 mx-auto max-w-[min(640px,100%)] pt-0">
        ...
        <p
          className={`font-caps mt-3 px-2 text-[clamp(9px,0.3vw+0.45rem,11px)] leading-[1.5] tracking-[2.2px] text-gold-light/65 uppercase transition-opacity duration-1000 ease-out delay-[600ms] motion-reduce:opacity-100 motion-reduce:transition-none ${
            visible ? "opacity-100" : "opacity-0"
          }`}
        >
          {footer.closing}
